...
**How a day is judged** - A day is a Vedic day, running from sunrise to the next sunrise. So a favorable Wednesday runs from Wednesday's sunrise until Thursday's sunrise, and 3 AM on Thursday still counts as Wednesday. Within that day, the script walks the nakshatram timeline. A stretch is favorable when:
- its nakshatram is favorable relative to the birth nakshatram, AND
- the **Tamil Yogam for (that weekday, that nakshatram)** is Siddha or Amrutha.

The Tamil Yogam is **computed from the Pambu (Vakya) Panchangam weekday × nakshatram chart** (see references/domain_knowledge.md), not taken from any website. Websites publish Tamil Yogam from differing tables, so their values are kept only as a reference.

**Data sources and fallback** - Each day's nakshatram timings and sunrise come from www.drikpanchang.com. If it is CAPTCHA-blocked or unreachable, the script falls back to www.prokerala.com (`https://www.prokerala.com/astrology/tamil-panchangam/YYYY-month-DD.html?loc=<geoname-id>`) for that day. Both sites agree on nakshatram timings, and their sunrise times differ by only a few minutes. Since the yogam comes from the chart, falling back does not change which days qualify. Once a site is blocked it isn't retried for the rest of the run, and `--no-fallback` uses drikpanchang.com only. If the output reports fallback dates, mention them briefly.

**Step 3 : Report to user** - 
a) The above step (Step 2) runs through complex calculation by finding intersection of favorable days of the week (Vedic days, sunrise to sunrise) AND favorable nakshatrams relative to birth nakshatram AND favorable Tamil Yogam (Siddha or Amrutha, looked up from the weekday × nakshatram chart). When combination of all three of them exists for a given day, that narrow window of time becomes a favorable time for that person. The above step should produce the results for the person under a per-person, per-city subfolder `{person}/{city}/` of either the user provided 'output_dir' argument or the '{person}_output_dir' directory relative to the user's current working directory (e.g. `Sai_output_dir/Sai/Chennai/`). Keeping each city in its own subfolder means running the same person for a second city does not overwrite the first city's results.
...
3. **Siddha or Amrutha Tamil Yogam** -- during that common favorable nakshatram, the Tamil Yogam from the chart for *each* person's own Vedic weekday (in their own city) is Siddha or Amrutha (not Marana).
...
**Report** - Very short common windows (a few minutes) are real but usually impractical; point out the longer ones. Show each person's individual table first (per assets/output_template.md), then the joint table from the "Group (couple) template" section of assets/output_template.md. If the persons have no common favorable weekday on any calendar, or the window yields no overlap, say so plainly and suggest a longer window.
